...
use Magento\NegotiableQuoteTemplate\Api\Template\ManagementInterface as TemplateManagementInterface;
...
class BuyerGenerateNegotiableQuoteFromTemplateTest extends WebapiAbstract
{
     ...
     */
    private $quoteTemplateManagement;
    ...
    protected function setUp(): void
    {
        ...
        $this->quoteTemplateManagement = $objectManager->get(TemplateManagementInterface::class);
    }
    ...
    public function testBuyerGenerateQuoteFromNegotiableQuoteTemplate(): void
    {
        $negotiableQuote = DataFixtureStorageManager::getStorage()->get('negotiable_quote');
        /** @var CartInterface $originalQuote */
        $originalQuote = $this->quoteRepository->get($negotiableQuote->getQuoteId());
        ...
        $serviceInfo = [
            'rest' => [
                'resourcePath' => '/V1/negotiableQuoteTemplate/generateQuote',
                'httpMethod' => Request::HTTP_METHOD_POST,
                'token' => $token
            ]
        ];

        $linkedQuoteId = $this->_webApiCall($serviceInfo, ['templateId' => $template->getTemplateId()]);
        $parentTemplate = $this->quoteTemplateManagement->getParentTemplate($linkedQuoteId);
        /** @var CartInterface $linkedQuote */
        $linkedQuote = $this->quoteRepository->get($linkedQuoteId);

        $this->assertEquals(true, $this->quoteTemplateManagement->isQuoteLinked($linkedQuoteId));
        $this->assertEquals($template->getTemplateId(), $parentTemplate->getTemplateId());
        $this->assertEquals($originalQuote->getCustomerId(), $linkedQuote->getCustomerId());
        $this->assertEquals($originalQuote->getGrandTotal(), $linkedQuote->getGrandTotal());
        $this->assertEquals($originalQuote->getItemsQty(), $linkedQuote->getItemsQty());
        foreach ($originalQuote->getItems() as $key => $item) {
            $linkedItem = $linkedQuote->getItems()[$key];
            $this->assertEquals($item->getSku(), $linkedItem->getSku());
            $this->assertEquals($item->getQty(), $linkedItem->getQty());
        }
    }
     ...
     * @param string $status
     * @param bool $expectException
     * @dataProvider dataTestTemplateStatusGenerateQuote
     */
    #[
        Config('btob/website_configuration/company_active', 1),
        Config('btob/website_configuration/negotiablequote_active', 1),
    ...
    public function testBuyerGenerateQuoteNegotiableQuoteTemplateStatusChecks(
        string $status,
     ...
     * @return array[]
     */
    public static function dataTestTemplateStatusGenerateQuote(): array
    {
        return [
