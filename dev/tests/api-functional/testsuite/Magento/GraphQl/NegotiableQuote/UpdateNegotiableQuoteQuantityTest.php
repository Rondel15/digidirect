...
class UpdateNegotiableQuoteQuantityTest extends GraphQlAbstract
{
     ...
     * @return array
     */
    public static function invalidQuoteIdDataProvider(): array
    {
        return [
            'missing_quote_uid' => [
                '',
    ...
    public function testUpdateNegotiableQuoteQuantitiesNonNegotiable(): void
    {
        ...
        $this->expectException(Exception::class);
        $this->expectExceptionMessage('The quotes with the following UIDs are not negotiable: ' . $cartId);

        $query = $this->getQuery($cartId, base64_encode('1'), 5.00);
    ...
    public function testUpdateNegotiableQuoteQuantitiesInvalidQuantities(float $qty, string $errorMessage): void
    {
        ...
        // This try/catch block is added to preserve for backward compatibility with 2.4.7-p4-develop versions.
        try {
            $this->graphQlMutation($query, [], '', $this->getHeaderMap());
        } catch (\Exception $exception) {
            $this->expectException(Exception::class);
            if (str_contains($exception->getMessage(), 'The requested qty is not available')) {
     ...
     * @return array
     */
    public static function invalidQuantitiesDataProvider(): array
    {
        return [
