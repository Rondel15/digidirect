...
namespace Magento\GraphQl\NegotiableQuoteDuplicate;
...
class DuplicateNegotiableQuoteTest extends GraphQlAbstract
{
    /**
    ...
    public function testDuplicateNegotiableQuote()
    {
        ...
        $duplicatedQuoteUid = str_shuffle($maskedQuoteId);
        $response = $this->graphQlMutation(
            $this->getMutation($maskedQuoteId, $duplicatedQuoteUid),
            [],
            '',
            ['Authorization' => sprintf('Bearer %s', $customerToken)]
        );

        $this->assertArrayHasKey('uid', $response['duplicateNegotiableQuote']['quote']);
        $this->assertEquals($duplicatedQuoteUid, $response['duplicateNegotiableQuote']['quote']['uid']);
    }

    /**
     * Returns GraphQl Query string to duplicate a negotiable quote.
     *
     * @param string $quoteId
     * @param string $duplicatedQuoteUid
     * @return string
     */
    private function getMutation(string $quoteId, string $duplicatedQuoteUid): string
    {
        return <<<MUTATION
mutation {
  duplicateNegotiableQuote(input: {quote_uid: "{$quoteId}", duplicated_quote_uid: "{$duplicatedQuoteUid}"}) {
    quote {
      uid
    }
  }
