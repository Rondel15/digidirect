...
class SetNegotiableQuoteTemplateShippingAddressTest extends GraphQlAbstract
{
    /**
    ...
    public function testSetNegotiableQuoteTemplateShippingAddress()
    {
        $customer = DataFixtureStorageManager::getStorage()->get('customer');
        $shippingAddresses = $customer->getAddresses();
        $shippingAddressUid = base64_encode(array_pop($shippingAddresses)->getId());
        ...
        $response = $this->graphQlMutation(
            $this->getMutation($templateId, $shippingAddressUid),
            [],
            '',
        ...
        $this->assertArrayHasKey(
            'shipping_addresses',
            $response['setNegotiableQuoteTemplateShippingAddress']
        );
        $expectedAddressData = [
            'firstname' => 'John',
            'lastname' => 'Doe',
        ...
        $this->assertEquals(
            $expectedAddressData,
            $response['setNegotiableQuoteTemplateShippingAddress']['shipping_addresses'][0]
        );
    }
     ...
     * @return string
     */
    private function getMutation(int $templateId, string $customerAddressUid): string
    {
        return <<<MUTATION
mutation
  {
