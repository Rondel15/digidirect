...
class CancelNegotiableQuoteTemplateTest extends GraphQlAbstract
{
    /**
    ...
    public function testCancelNegotiableQuoteTemplate()
    {
        ...
        $cancellationComment = 'Cancel this template';
        $response = $this->graphQlMutation(
            $this->getMutation($templateId, $cancellationComment),
            [],
            '',
            ['Authorization' => sprintf('Bearer %s', $customerToken)]
        );
        $expectedStatusLabel = $this->statusProvider->getStatusLabel(TemplateInterface::STATUS_CANCELED);
        $this->assertEquals(
            $expectedStatusLabel,
            $response['cancelNegotiableQuoteTemplate']['status']
        );
        $history = $response['cancelNegotiableQuoteTemplate']['history'];
        $actualCancellationRecord = array_pop($history)['changes']['custom_changes'];
        $expectedCancellationComment = sprintf('"%s"', $cancellationComment);
        $this->assertEquals(
            $expectedCancellationComment,
            $actualCancellationRecord['new_value']
        );
    }
     ...
     * @return string
     */
    private function getMutation(int $templateId, string $cancellationComment): string
    {
        return <<<MUTATION
mutation
  {
    cancelNegotiableQuoteTemplate(
    input: {template_id: "$templateId", cancellation_comment: "$cancellationComment"}
)
   {
       template_id
       status
