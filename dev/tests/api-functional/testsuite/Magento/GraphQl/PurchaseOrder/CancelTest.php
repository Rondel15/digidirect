...
/**
 * Test cancel purchase order mutation
 */
#[
...
class CancelTest extends GraphQlAbstract
{
    private const QUERY = <<<QRY
mutation {
  cancelPurchaseOrders(input: {purchase_order_uids: [%s]}) {
    purchase_orders {
      status
    }
    errors {
...
QRY;
     ...
     */
    public function testCancelPurchaseOrderInPendingStatus()
    {
     ...
     * @throws LocalizedException
     */
    public function testCancelPurchaseOrderInApprovalRequiredStatus()
    {
        /** @var PurchaseOrderInterface $purchaseOrder */
        $purchaseOrder = DataFixtureStorageManager::getStorage()->get('purchase_order');
     ...
     * @throws LocalizedException
     */
    public function testAttemptToCancelPurchaseOrderInCanceledStatus()
    {
        /** @var PurchaseOrderInterface $purchaseOrder */
        $purchaseOrder = DataFixtureStorageManager::getStorage()->get('purchase_order');
        $this->purchaseOrderManagement->cancelPurchaseOrder($purchaseOrder);

        $expectedMessage = sprintf(
            "Action 'cancel' is not allowed for purchase order %s.",
     ...
     */
    public function testAttemptToCancelPurchaseOrderInApprovedStatus()
    {
        /** @var PurchaseOrderInterface $purchaseOrder */
        $purchaseOrder = DataFixtureStorageManager::getStorage()->get('purchase_order');
     ...
     * @throws LocalizedException
     */
    public function testAttemptToCancelPurchaseOrderInRejectedStatus()
    {
        /** @var PurchaseOrderInterface $purchaseOrder */
        $purchaseOrder = DataFixtureStorageManager::getStorage()->get('purchase_order');
        $this->purchaseOrderManagement->rejectPurchaseOrder($purchaseOrder);
     ...
     * @throws LocalizedException
     */
    public function testAttemptToValidateNonExistingPurchaseOrders()
    {
        $nonExistingPurchaseOrdersIds = ["90000001", "90000002"];

        $expectedMessage = "Action 'cancel' - purchase order with requested ID=%s not found";
     ...
     * @throws Exception
     */
    public function testAttemptToCancelUnauthenticatedUser()
    {
