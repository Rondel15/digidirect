...
QUERY;
     ...
     */
    private function verifyResourceAccess(array $permissions, $query): void
    {
        $this->denyResourceAccess($permissions);
    ...
    /**
     * Deny access for the default role
     *
     ...
     */
    private function denyResourceAccess(array $permissions) : void
    {
     ...
     * @magentoConfigFixture btob/website_configuration/company_active 1
     */
    public function testGetCompanyRolesWithoutPermission(): void
    {
        $deniedPermissions = [
            'Magento_Company::roles_view',
            'Magento_Company::roles_edit'
        ];
        $this->denyResourceAccess($deniedPermissions);
...
QUERY;
        $response = $this->graphQlQuery(
            ...
            [],
            '',
            $this->customerAuthenticationHeader->execute('veronica.costello@example.com')
        );
        self::assertArrayHasKey('name', $response['company']['roles']['items'][0]);
        self::assertArrayHasKey('id', $response['company']['roles']['items'][0]);
    }
}
