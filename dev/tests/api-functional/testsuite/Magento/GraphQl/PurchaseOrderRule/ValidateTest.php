...
class ValidateTest extends GraphQlAbstract
{
...
QRY;
     ...
     */
    public function testValidatePendingPurchaseOrdersWhenNoRulesApply()
    {
     ...
     */
    public function testAttemptToValidateApprovedPurchaseOrders()
    {
        /** @var PurchaseOrderInterface $purchaseOrder */
        $purchaseOrder = DataFixtureStorageManager::getStorage()->get('purchase_order');
        ...
        $expectedMessage = sprintf(
            "Purchase order %d cannot be validated.",
    ...
    public function testAttemptToValidateNonExistingPurchaseOrders()
    {
        ...
        $expectedMessage = "No such entity with entity_id = %s";
        foreach ($nonExistingPurchaseOrdersIds as $nonExistingPurchaseOrdersId) {
            $expectedErrors[] = [
                "message" => sprintf($expectedMessage, $nonExistingPurchaseOrdersId),
                "type" => "NOT_FOUND"
            ];
        }
     ...
     * @throws Exception
     */
    public function testAttemptToValidateUnauthenticatedUser()
    {
