...
class CloseNegotiableQuotesTest extends GraphQlAbstract
{
    ...
    public function testMutation(
        ...
        if ($errors != null) {
            foreach ($quoteMasks as $key => $quoteMask) {
                ...
                $this->assertOperationResultFailure(
                    $response['closeNegotiableQuotes']['operation_results'][$key],
                    $quoteMask,
        ...
        if ($errors === null) {
            ...
            $this->assertArrayHasKey('result_status', $response['closeNegotiableQuotes']);
            $this->assertArrayHasKey('closed_quotes', $response['closeNegotiableQuotes']);
            $this->assertEquals(BatchResult::STATUS_SUCCESS, $response['closeNegotiableQuotes']['result_status']);
            $this->assertCount(count($quoteMasks), $response['closeNegotiableQuotes']['closed_quotes']);

            foreach ($quoteMasks as $key => $quoteMask) {
                $this->assertArrayHasKey($key, $response['closeNegotiableQuotes']['closed_quotes']);
                $this->assertArrayHasKey('uid', $response['closeNegotiableQuotes']['closed_quotes'][$key]);
                $this->assertEquals($quoteMask, $response['closeNegotiableQuotes']['closed_quotes'][$key]['uid']);
            }
        }
    }
