...
/**
 * Test quote renaming in different company scopes.
 */
class RenameNegotiableQuoteTest extends GraphQlAbstract
{
    ...
    public function testMutation(
        ...
        if (!$exception) {
            $this->assertNotEmpty($response['renameNegotiableQuote']);
            $this->assertArrayHasKey('quote', $response['renameNegotiableQuote']);
            ...
            $this->assertEquals($quoteMask, $response['renameNegotiableQuote']['quote']['uid']);
            $this->assertEquals($quoteNewName, $response['renameNegotiableQuote']['quote']['name']);
        }
    }
    ...
    {
        return [
            ['NQ_One_mask', 'Renamed_NQ_One_mask', null, null],
            ['NQ_One_mask', 'Renamed_NQ_One_mask', 'company_one', null],
            ['NQ_One_mask', 'Renamed_NQ_One_mask', 'company_two', 'Could not find a quote with the specified UID.'],
            ['NQ_Two_mask', 'Renamed_NQ_Two_mask', null, 'Could not find a quote with the specified UID.'],
            ['NQ_Two_mask', 'Renamed_NQ_Two_mask', 'company_one', 'Could not find a quote with the specified UID.'],
            ['NQ_Two_mask', 'Renamed_NQ_Two_mask', 'company_two', null],
        ];
    }
     ...
     * @return string
     */
    private function getMutation(string $quoteId, string $quoteName): string
    {
        return <<<MUTATION
mutation {
  renameNegotiableQuote(input: {
      quote_uid: "{$quoteId}"
      quote_name: "{$quoteName}"
      quote_comment: "Renaming Quote to {$quoteName}"
  }) {
    quote {
      uid
      name
