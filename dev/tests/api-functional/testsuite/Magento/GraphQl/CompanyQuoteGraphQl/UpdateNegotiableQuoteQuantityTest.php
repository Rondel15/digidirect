...
class UpdateNegotiableQuoteQuantityTest extends GraphQlAbstract
{
    ...
    public function testMutation(
        ...
        if (!$exception) {
            ...
            $this->assertArrayHasKey('uid', $response['updateNegotiableQuoteQuantities']['quote']);
            $this->assertEquals($quoteMask, $response['updateNegotiableQuoteQuantities']['quote']['uid']);
            $this->assertNotEmpty($response['updateNegotiableQuoteQuantities']['quote']['items']);
            $this->assertCount(1, $response['updateNegotiableQuoteQuantities']['quote']['items']);
            $this->assertArrayHasKey(0, $response['updateNegotiableQuoteQuantities']['quote']['items']);
            $this->assertArrayHasKey('id', $response['updateNegotiableQuoteQuantities']['quote']['items'][0]);
            $this->assertEquals(
                $item->getItemId(),
                $response['updateNegotiableQuoteQuantities']['quote']['items'][0]['id']
            );
            $this->assertArrayHasKey('quantity', $response['updateNegotiableQuoteQuantities']['quote']['items'][0]);
            $this->assertEquals(
                $quantity,
     ...
     * @return string
     */
    private function getMutation(string $quoteId, string $quoteItemId, float $quantity): string
    {
        return <<<MUTATION
mutation {
