...
class SendNegotiableQuoteForReviewTest extends GraphQlAbstract
{
    ...
    public function testMutation(
        ...
        if (!$exception) {
            $this->assertNotEmpty($response['sendNegotiableQuoteForReview']);
            $this->assertArrayHasKey('quote', $response['sendNegotiableQuoteForReview']);
            $this->assertArrayHasKey('uid', $response['sendNegotiableQuoteForReview']['quote']);
            $this->assertEquals($quoteMask, $response['sendNegotiableQuoteForReview']['quote']['uid']);
            $this->assertArrayHasKey('status', $response['sendNegotiableQuoteForReview']['quote']);
            $this->assertEquals('SUBMITTED', $response['sendNegotiableQuoteForReview']['quote']['status']);
        }
    }
     ...
     * @return string
     */
    private function getMutation(string $quoteId, string $comment): string
    {
        return <<<MUTATION
mutation {
