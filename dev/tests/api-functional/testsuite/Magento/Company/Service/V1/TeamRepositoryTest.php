...
class TeamRepositoryTest extends WebapiAbstract
{
    ...
    public const SERVICE_READ_NAME = 'companyTeamRepositoryV1';
     ...
     *
     * @param int|null $contextCompanyId
     * @return void
     * @magentoApiDataFixture Magento/NegotiableQuote/_files/company_with_customer_for_quote.php
     * @dataProvider hierarchyGetDataProvider
     */
    public function testCreateTeam(?int $contextCompanyId)
    {
        ...
        if ($contextCompanyId) {
            if (TESTS_WEB_API_ADAPTER === self::ADAPTER_SOAP) {
                $this->markTestSkipped(
                    'Test works only for REST adapter because there is no support of headers in SOAP requests'
                );
            }
            $serviceInfo['rest']['headers'] = ['X-Adobe-Company: ' . $contextCompanyId];
        }
     ...
     * @return array
     */
    public function hierarchyGetDataProvider(): array
    {
        return [
            'context company id is null' => [null],
            'context company id is wrong' => [-1],
        ];
    }
    ...
    public function testDeleteTeamWithChildren(): void
    {
        ...
        $this->expectExceptionMessage('This team has active users or teams assigned to it and cannot be deleted.');
