...
Resolver::getInstance()->requireDataFixture('Magento/Bundle/_files/quote_with_bundle_and_options.php');
...
$companyQuoteLinkFactory = $objectManager->get(CompanyQuoteLinkFactory::class);
$quote = $quoteFactory->create();
$quoteResource->load($quote, 'test_cart_with_bundle_and_options', 'reserved_order_id');
...
foreach ($quote->getAddressesCollection() as $address) {
    ...
    $address
        ...
        ->setFirstname($companySubordinate->getFirstname())
        ->setLastname($companySubordinate->getLastname())
        ->setStreet('123 Real Street')
        ->setCity('Austin')
        ->setCountryId('US')
        ->setRegionId(1)
        ->setPostcode('11111')
        ->setTelephone('11111')
        ->setCustomerId($companySubordinate->getId())
        ->setQuote($quote)
        ->save();
}
...
$companyQuoteLink = $companyQuoteLinkFactory->create();
$companyQuoteLink->setQuoteId($quote->getId())->setCompanyId($company->getId())->save();
