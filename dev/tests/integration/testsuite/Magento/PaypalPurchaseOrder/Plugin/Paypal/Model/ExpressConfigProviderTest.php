...
class ExpressConfigProviderTest extends TestCase
{
    ...
    public function testAfterGetConfigNonPurchaseOrderCreator()
    {
        // Configure the company to use purchase orders
        $company = $this->getCompanyByName('Magento');
        $this->setAvailablePaymentMethodsForCompany(['paypal_express','paypal_express_bml'], $company);
        $this->setCompanyPurchaseOrderConfig($company, true);
    ...
    /**
     * Set available payment methods for company
     *
     ...
     */
    private function setAvailablePaymentMethodsForCompany(array $paymentMethodCodes, CompanyInterface $company)
    {
        $companyExtensionAttributes = $company->getExtensionAttributes();
        $methods = $companyExtensionAttributes->getAvailablePaymentMethods();
        $methodsArray = $methods ? explode(',', $methods) : [];
        $newMethods = implode(',', array_merge($methodsArray, $paymentMethodCodes));
        $companyExtensionAttributes->setAvailablePaymentMethods($newMethods);
        $company->getExtensionAttributes($companyExtensionAttributes);
