 ...
 */
class GridToXmlTest extends AbstractBackendController
{
    /**
    ...
    {
        ob_start();
        $response = $this->getXmlExportResponse();
        $this->assertEquals(200, $response->getHttpResponseCode());
        $this->assertCorrectContentHeadersInResponse($response);

        $xmlContents = ob_get_clean();
        $actualRows = $this->mapXmlContentsToArray($xmlContents);
        $expectedRows = $this->getExpectedRowsFromXml();
    ...
    public function testExportOfSelectedCompanies()
    {
        ...
        $response = $this->getXmlExportResponse($selectedCompanyIds);
        $this->assertEquals(200, $response->getHttpResponseCode());
        $this->assertCorrectContentHeadersInResponse($response);
        ...
        $expectedRows = array_slice($this->getExpectedRowsFromXml(), 3, 2); // expect 3rd and 4th company only
     ...
     */
    private function getXmlExportResponse(array $selectedCompanyIds = [])
    {
        ...
        $this->dispatch('backend/mui/export/gridToXml/?namespace=company_listing');
    ...
    private function assertCorrectContentHeadersInResponse(ResponseInterface $response)
    {
        ...
        $this->assertEquals('attachment; filename="export.xml"', $headersArr['Content-Disposition']);
    }
     ...
     * @return array
     */
    private function mapXmlContentsToArray(string $xmlContents)
    {
        $xml = simplexml_load_string($xmlContents);

        $rows = [];

        $xmlRows = $xml->Worksheet->Table->Row;

        foreach ($xmlRows as $xmlRow) {
            $row = [];
            foreach ($xmlRow->Cell as $cell) {
                $row[] = (string) $cell->Data;
            }
     ...
     */
    private function getExpectedRowsFromXml()
    {
        $companies = $this->getCompaniesCreatedInFixture();
        ...
        $expectedData = [
            [
                ...
                'ZIP' => 'postcode 1',
                'City' => 'city 1',
                'Group/Shared Catalog' => 'Shared Catalogs Default (General)',
                'Street Address' => 'street 1',
                'Company Admin' => 'First Name 1 Last Name 1',
                'Job Title' => 'Job Title 1',
                'Work Phone Number' => 'Work Phone Number 1',
            ...
            [
                ...
                'Work Phone Number' => 'Work Phone Number 2',
            ...
            [
                ...
                'Work Phone Number' => 'Work Phone Number 3',
            ...
            [
                ...
                'Work Phone Number' => 'Work Phone Number 4',
            ...
            [
                ...
                'Work Phone Number' => 'Work Phone Number 5',
