...
{
    private const QUOTE_ID = 2;

    private const COMPANY_ID = 11;
    ...
    public function testExecute(): void
    {
        ...
        $this->fixtureModel->method('getValue')
            ->willReturnCallback(
                function ($arg1, $arg2) {
                    if ($arg1 == 'negotiable_quotes' && $arg2 == 0) {
                        return 100;
                    } elseif ($arg1 == 'customers' && $arg2 == 0) {
                        return 42;
                    }
                }
            );
        ...
        $select->expects($this->atLeastOnce())->method('from')
            ->willReturnCallback(
                function ($arg1, $arg2) use ($select) {
                    if ($arg1 === 'negotiable_quote' && $arg2 == 'COUNT(*)') {
                        return $select;
                    } elseif ($arg1 === ['qa' => 'quote_address']) {
                        return $select;
                    } elseif ($arg1 === 'quote_item' || $arg1 === ['quote_item' => 'quote_item']) {
                        return $select;
                    }
                }
            );
        ...
        $select->expects($this->atLeastOnce())
            ...
            ->willReturnCallback(
                function ($arg1, $arg2) use ($select) {
                    if ($arg1 == 'qa.quote_id IN (?)' && $arg2 == [2]) {
                        return $select;
                    } elseif ($arg1 == 'quote_id=?' && $arg2 == 2) {
                        return $select;
                    } elseif ($arg1 == 'parent_item_id=?' && $arg2 == 44) {
                        return $select;
                    } elseif ($arg1 == 'quote_item.quote_id = 2') {
                        return $select;
                    }
                }
            );
    ...
    private function mockConnection(): MockObject
    {
        ...
        $this->resources->expects($this->atLeastOnce())
            ...
            ->willReturnCallback(
                function ($arg) {
                    if ($arg === 'negotiable_quote') {
                        return 'negotiable_quote';
                    } elseif ($arg === 'quote_address') {
                        return 'quote_address';
                    } elseif ($arg === 'quote_item') {
                        return 'quote_item';
                    } elseif ($arg === 'negotiable_quote_item') {
                        return 'negotiable_quote_item';
                    } elseif ($arg === 'negotiable_quote_history') {
                        return 'negotiable_quote_history';
                    } elseif ($arg === 'negotiable_quote_grid') {
                        return 'negotiable_quote_grid';
                    } elseif ($arg === 'quote') {
                        return 'quote';
                    }
                }
        ...
        $connection->expects($this->atLeastOnce())
            ...
            ->willReturnCallback(function ($arg1, $arg2, $arg3) {
                if ($arg1 === 'negotiable_quote_history' &&
                    $arg2 == $this->getNegotiableQuoteHistorySampleData() &&
                    $arg3 == array_keys($this->getNegotiableQuoteHistorySampleData())) {
                    return 1;
                } elseif ($arg1 === 'negotiable_quote_grid' &&
                    $arg2 == $this->getNegotiableQuoteGridSampleData() &&
                    $arg3 == array_keys($this->getNegotiableQuoteGridSampleData())) {
                    return 1;
                } elseif ($arg1 == 'quote' &&
                    $arg2 == [$this->getQuoteSampleData()] &&
                    $arg3 == array_keys($this->getQuoteSampleData())) {
                    return 1;
                }
            });
        $connection->expects($this->atLeastOnce())
    ...
    private function prepareQuoteCollection($collectionSize): QuoteCollection
    {
        ...
        $quoteCollection->expects($this->atLeastOnce())
            ...
            ->willReturnCallback(
                function ($args) use ($quoteCollection) {
                    if ($args === ['entity_id', 'store_id']) {
                        return $quoteCollection;
                    } elseif ($args === ['checkout_method', 'guest']) {
                        return $quoteCollection;
                    }
                }
            );
